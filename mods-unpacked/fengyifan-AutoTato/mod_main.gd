...
#
# 当前模块：数据层 + 决策器层 + Bridge + 商店 hook + 升级 hook + 箱子 hook。
#   Script Extension 已挂到 vanilla base_shop 与 upgrades_ui。后者通过 _items_container.visible
#   分支同时处理升级 4 选 1 (decide_upgrade) 与箱子单物品 (decide_chest_item)。
...
# ----------------------------------------------------------------------------
# 数据层文件路径（暂未接入运行时，仅做路径单点配置 + preload 校验）
# ----------------------------------------------------------------------------
# 这些常量目前的作用：
...
const PATH_DATA_SMOKE_TEST    := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/data_smoke_test.gd"

# ----------------------------------------------------------------------------
# 决策器层文件路径（同样仅 preload 校验，尚未接入 vanilla hook）
# ----------------------------------------------------------------------------
...
const PATH_DECISION_SMOKE_TEST    := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/decision_smoke_test.gd"

# ----------------------------------------------------------------------------
# Bridge 层文件路径
# ----------------------------------------------------------------------------
const PATH_BRIDGE           := "res://mods-unpacked/fengyifan-AutoTato/autotato/runtime/bridge.gd"
const PATH_BRIDGE_SMOKE_TEST    := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/bridge_smoke_test.gd"
...
const PATH_SHOP_HOOK_SMOKE_TEST    := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/shop_hook_smoke_test.gd"
...
const PATH_UPGRADE_HOOK_SMOKE_TEST  := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/upgrade_hook_smoke_test.gd"

# ----------------------------------------------------------------------------
# ConfigManager 配置持久化层
# ----------------------------------------------------------------------------
const PATH_CONFIG_MANAGER   := "res://mods-unpacked/fengyifan-AutoTato/autotato/runtime/config_manager.gd"
const PATH_PERSISTENCE_SMOKE_TEST    := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/persistence_smoke_test.gd"
...
const PATH_CONFIG_PANEL_SMOKE_TEST       := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/config_panel_smoke_test.gd"
const PATH_BRIDGE_V6_SMOKE_TEST      := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/bridge_v6_smoke_test.gd"
const PATH_GAMEPAD_SMOKE_TEST         := "res://mods-unpacked/fengyifan-AutoTato/autotato/dev/gamepad_smoke_test.gd"
...
# UI 类 preload 校验 (强制启动期解析, 写错路径或语法错立刻报错)
...
# ----------------------------------------------------------------------------
# 推荐用环境变量临时启用，跑完不留痕：
#   数据层:  AUTOTATO_SMOKE=1          ./Brotato.x86_64   (兼容旧名)
#   决策层:  AUTOTATO_DECISION_SMOKE=1 ./Brotato.x86_64
#   Bridge:  AUTOTATO_BRIDGE_SMOKE=1   ./Brotato.x86_64
#   商店Hook: AUTOTATO_SHOP_HOOK_SMOKE=1 ./Brotato.x86_64
#   升级Hook: AUTOTATO_UPGRADE_HOOK_SMOKE=1 ./Brotato.x86_64
# 多个开关可以叠加，验证全栈
const DEV_RUN_DATA_SMOKE := false
const DEV_RUN_DECISION_SMOKE := false
const DEV_RUN_BRIDGE_SMOKE := false
const DEV_RUN_SHOP_HOOK_SMOKE := false
const DEV_RUN_UPGRADE_HOOK_SMOKE := false
const DEV_RUN_PERSISTENCE_SMOKE := false
const DEV_RUN_CONFIG_PANEL_SMOKE := false
const DEV_RUN_BRIDGE_V6_SMOKE := false
const DEV_RUN_GAMEPAD_SMOKE := false
...
# Bridge 全局实例。在 _init() 末尾 new 出来并注册到 Engine.set_meta，
# Hook 层通过 AT_Bridge.get_global() 拿到。
...
func _init() -> void:
	...
	# 创建 Bridge 实例并注册到 Engine 元数据
	# Hook 层通过 AT_Bridge.get_global() 拿到，一行接入
...
func _ready() -> void:
	ModLoaderLog.info("AutoTato 已加载（数据+决策+Bridge+Hook+持久化+UI）", LOG_NAME)

	# 开发期烟雾测试：常量开关 + 环境变量 双触发
	# 用 deferred 避免在 _ready 链上做长 IO，让其他 mod 先加载完
	# AUTOTATO_SMOKE 兼容旧名（等同 AUTOTATO_DATA_SMOKE）
	var run_data := DEV_RUN_DATA_SMOKE \
		or OS.has_environment("AUTOTATO_SMOKE") \
		or OS.has_environment("AUTOTATO_DATA_SMOKE")
	var run_decision := DEV_RUN_DECISION_SMOKE or OS.has_environment("AUTOTATO_DECISION_SMOKE")
	var run_bridge := DEV_RUN_BRIDGE_SMOKE or OS.has_environment("AUTOTATO_BRIDGE_SMOKE")
	var run_shop_hook := DEV_RUN_SHOP_HOOK_SMOKE or OS.has_environment("AUTOTATO_SHOP_HOOK_SMOKE")
	var run_upgrade_hook := DEV_RUN_UPGRADE_HOOK_SMOKE or OS.has_environment("AUTOTATO_UPGRADE_HOOK_SMOKE")
	var run_persistence := DEV_RUN_PERSISTENCE_SMOKE or OS.has_environment("AUTOTATO_PERSISTENCE_SMOKE")
	var run_config_panel := DEV_RUN_CONFIG_PANEL_SMOKE or OS.has_environment("AUTOTATO_CONFIG_PANEL_SMOKE")
	var run_bridge_v6 := DEV_RUN_BRIDGE_V6_SMOKE or OS.has_environment("AUTOTATO_BRIDGE_V6_SMOKE")
	var run_gamepad := DEV_RUN_GAMEPAD_SMOKE or OS.has_environment("AUTOTATO_GAMEPAD_SMOKE")

	if run_data:
		call_deferred("_run_smoke_test", PATH_DATA_SMOKE_TEST, "数据层")
	if run_decision:
		call_deferred("_run_smoke_test", PATH_DECISION_SMOKE_TEST, "决策层")
	if run_bridge:
		call_deferred("_run_smoke_test", PATH_BRIDGE_SMOKE_TEST, "Bridge")
	if run_shop_hook:
		call_deferred("_run_smoke_test", PATH_SHOP_HOOK_SMOKE_TEST, "商店Hook")
	if run_upgrade_hook:
		call_deferred("_run_smoke_test", PATH_UPGRADE_HOOK_SMOKE_TEST, "升级Hook")
	if run_persistence:
		call_deferred("_run_smoke_test", PATH_PERSISTENCE_SMOKE_TEST, "持久化")
	if run_config_panel:
		call_deferred("_run_smoke_test", PATH_CONFIG_PANEL_SMOKE_TEST, "配置面板")
	if run_bridge_v6:
		call_deferred("_run_smoke_test", PATH_BRIDGE_V6_SMOKE_TEST, "BridgeV6")
	if run_gamepad:
		call_deferred("_run_smoke_test", PATH_GAMEPAD_SMOKE_TEST, "手柄")
