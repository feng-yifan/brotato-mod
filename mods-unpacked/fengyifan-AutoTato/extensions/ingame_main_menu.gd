...
const PATH_CONFIG_PANEL := "res://mods-unpacked/fengyifan-AutoTato/ui/config_panel.tscn"
...
func _on_autotato_config_pressed() -> void:
	...
	# 记录打开面板的玩家, 供面板做 coop 按键归属 (ESC/B/Start/肩键只响应归属玩家).
	# vanilla IngameMainMenu.player_index 由 PauseMenu.init 设置, 此处可直接读.
	_config_panel._opener_player_index = self.player_index
	_config_panel.show()
